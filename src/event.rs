...
pub trait DomainEvent<'a, T: Hash + Eq, V> {
    ...
    /// data returns a hashmap of the underlying event data.  This allows domain events to change over time.
    /// If relevant event data was defined in the domain event's struct, then it would be tied to a specific type.
    /// With this pattern we instead keep domain events more loosely coupled, so we aren't tied closely to the type
    /// of the implementor for projections.
    fn data(&self) -> HashMap<String, V>;
}
